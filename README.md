...
The current R29.5 test package uses APK version `0.1.0.29.5-r1`. APK reserves
`-rN` for its integer package revision, so the product release is represented
as dotted `PKG_VERSION` components and the package revision is `r1`. The latest
...
find openwrt/bin -name 'aredn-multiwan-0.1.0.29.5-r1.apk' -print -exec sha256sum {} \;
```
